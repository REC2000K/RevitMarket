...
import { getEnrichedFamilies } from '@/data/families';
...
export function FamilyDetailPage({
  ...
  // Load families from static source (NO localStorage)
  useEffect(() => {
    const data = getEnrichedFamilies();
    setAllFamilies(data);
  }, []);
  ...
  const relatedFamilies = allFamilies
    .filter(
      (f) => f.category === family.category && f.id !== family.id
    )
    .slice(0, 3);
  ...
  const handleDownload = () => {
    // пока без backend/localStorage — просто UI эффект
              ...
              <div className="flex flex-wrap items-center gap-6 mb-8 pb-8 border-b border-gray-200">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[#6B7280]">
                    Добавлено{' '}
                    {new Date(family.dateAdded).toLocaleDateString('ru-RU')}
                  </span>
                </div>
